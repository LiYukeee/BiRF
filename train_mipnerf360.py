...
def main(
    ...
    from mipnerf360dataset.colmap_provider import ColmapDataset as NeRFDataset
    from mipnerf360dataset.GetConfig import get_config

    args_list = [
                "/home/liyuke/liyuke/code/BiRF/data/mipnerf360/{}".format(scene),
                "--workspace",
                "workspace/MGL_30dB/mipnerf360/{}".format(scene),
                "-O",
                "-MGL",
                "--data_format",
                "colmap",
    ...
    opt = get_config(args_list)
    train_dataset = NeRFDataset(opt, device=device, type='train').dataloader()
    test_dataset = NeRFDataset(opt, device=device, type='val').dataloader()
    ...
    for step in pbar:
        ...
        #     while True:
        #         try:
        #             batch = next(train_loader_iter)  # 获取下一个批次
        #             train_loader_batches.append(batch)  # 将批次添加到列表中
        #         except StopIteration:
        #             # 当没有更多批次时，迭代器会抛出 StopIteration 异常
        #             break
        # data = train_loader_batches[step % len(train_dataset)]
        if step % len(train_dataset) == 0:
            train_loader_iter = iter(train_dataset)  # 重置迭代器
        data = next(train_loader_iter)

        data['color_bkgd'] = torch.tensor([1., 1., 1.], device=device)
        render_bkgd = data["color_bkgd"]
        from lib.datasets.utils import Rays, namedtuple_map
        rays = Rays(origins=data['rays_o'], viewdirs=data['rays_d'])
        pixels = data["images"]
        ...
        def occ_eval_fn(x):
            if cone_angle > 0.0:
                ...
                camtoworlds_matrix = train_dataset._data.mvps
                origins = camtoworlds_matrix[camera_ids, :3, -1]
        ...
        if step == max_steps and step > 0:
            ...
            n_rendering_samples_each_image_list = []
            with torch.no_grad():
                ...
                for data in test_dataset:
                    ...
                    pixels = data["images"]
                    # rendering
                    rgb, acc, depth, n_rendering_samples_each_image = render_image(
                        radiance_field,
                        occupancy_grid,
                        rays,
                        scene_aabb,
                    ...
                    n_rendering_samples_each_image_list.append(n_rendering_samples_each_image)
                        
            psnr_avg = sum(psnr_list) / len(psnr_list)
            ssim_avg = sum(ssim_list) / len(ssim_list)
            lpips_avg = sum(lpips_list) / len(lpips_list)
            n_rendering_samples_each_image_avg = sum(n_rendering_samples_each_image_list) / len(n_rendering_samples_each_image_list)
            ...
            print(f"Rvaluation num of rendering samples for one image: {n_rendering_samples_each_image_avg}")
            ...
            results['size'] = round(total_size / 2 ** 20, 4)
            results['encoding_size'] = round(encoding_size / 2 ** 20, 4)
            results['network_size'] = round(network_size / 2 ** 20, 4)
            results['occgrid_size'] = round(occgrid_size / 2 ** 20, 4)
            results['num_samples_each_image'] = n_rendering_samples_each_image_avg
            results['image_size'] = str(data['images'].shape)
